...
import { Truck, Users, FileText, Package, Clock, DollarSign, LogOut, RefreshCw } from 'lucide-react';

const API_BASE = import.meta.env.VITE_API_URL || 'https://haulflow-production-575a.up.railway.app';

// Direct fetch to Railway (not the tenant api module)
...
export default function AdminDashboard() {
      ...
      <div className="min-h-screen bg-gray-950 text-white flex items-center justify-center px-6">
        <div className="w-full max-w-sm">
          <div className="text-center mb-8">
            ...
            <p className="text-gray-500 text-sm mt-1">Platform management dashboard</p>
          </div>
  ...
  const statsCards = stats ? [
    { label: 'Total Companies', value: stats.companies.total, icon: <Package className="w-4 h-4" />, color: 'text-blue-400' },
    { label: 'Active Subscriptions', value: stats.companies.active, icon: <DollarSign className="w-4 h-4" />, color: 'text-green-400' },
    { label: 'In Free Trial', value: stats.companies.inTrial, icon: <Clock className="w-4 h-4" />, color: 'text-yellow-400' },
    { label: 'Cancelled', value: stats.companies.cancelled, icon: <FileText className="w-4 h-4" />, color: 'text-red-400' },
    { label: 'Total Users', value: stats.totalUsers, icon: <Users className="w-4 h-4" />, color: 'text-purple-400' },
    { label: 'Total Drivers', value: stats.totalDrivers, icon: <Truck className="w-4 h-4" />, color: 'text-cyan-400' },
    { label: 'Total Loads', value: stats.totalLoads, icon: <Package className="w-4 h-4" />, color: 'text-orange-400' },
    { label: 'Total Invoices', value: stats.totalInvoices, icon: <FileText className="w-4 h-4" />, color: 'text-emerald-400' },
  ] : [];
    ...
    <div className="min-h-screen bg-gray-950 text-white font-sans">
      ...
      <div className="border-b border-gray-800 px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <span className="text-xs text-gray-600 bg-gray-900 px-2 py-0.5 rounded">v1</span>
        </div>
        <div className="flex items-center gap-3">
          <button
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-8">
            {statsCards.map(card => (
              <div key={card.label} className="bg-gray-900 border border-gray-800 rounded-xl p-4">
                <div className={`text-xs text-gray-500 mb-1 flex items-center gap-1.5 ${card.color}`}>
        ...
        {slots && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
            <div className="bg-green-950/30 border border-green-900 rounded-xl p-5">
              <h3 className="text-sm font-bold text-green-400 mb-3">🏷️ Founding Tier 1 — Free 1 Year (Android)</h3>
              <div className="flex items-center gap-4">
                <div className="flex-1">
                  <div className="w-full bg-gray-800 rounded-full h-3">
                    <div
                      className="bg-green-500 h-3 rounded-full transition-all"
                      style={{ width: `${(slots.founding_1yr_used / slots.founding_1yr_total) * 100}%` }}
                    />
                  </div>
                </div>
                <span className="text-sm font-bold text-green-400">
                  {slots.founding_1yr_used}/{slots.founding_1yr_total}
                </span>
              </div>
              <p className="text-xs text-gray-500 mt-1">{slots.founding_1yr_remaining} slots remaining</p>
            </div>

            <div className="bg-green-950/30 border border-green-900 rounded-xl p-5">
              <h3 className="text-sm font-bold text-green-400 mb-3">🏷️ Founding Tier 2 — Free 6 Months</h3>
              <div className="flex items-center gap-4">
                <div className="flex-1">
                  <div className="w-full bg-gray-800 rounded-full h-3">
                    <div
                      className="bg-green-400 h-3 rounded-full transition-all"
                      style={{ width: `${(slots.founding_6mo_used / slots.founding_6mo_total) * 100}%` }}
                    />
                  </div>
                </div>
                <span className="text-sm font-bold text-green-400">
                  {slots.founding_6mo_used}/{slots.founding_6mo_total}
                </span>
              </div>
              <p className="text-xs text-gray-500 mt-1">{slots.founding_6mo_remaining} slots remaining</p>
            </div>
          </div>
        )}
                        ...
                        <td className="px-4 py-3">
                          {c.stripe_customer_id ? (
                            <span className="text-xs text-green-400 font-mono">
                              cus_…{c.stripe_customer_id.slice(-6)}
                            </span>
                          ) : (
