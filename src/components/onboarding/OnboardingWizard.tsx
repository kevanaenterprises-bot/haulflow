        ...
        return (
                <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-brand-900 flex items-center justify-center p-4">
                        <div className="bg-slate-800 border border-slate-700 rounded-2xl shadow-2xl p-10 w-full max-w-md text-center">
                                  ...
                                  <h2 className="text-2xl font-bold text-white mb-2">You're All Set!</h2>h2>
                                  <p className="text-slate-400 mb-6">Your HaulFlow account has been configured. Welcome aboard.</p>p>
                                  <button onClick={() => window.location.href = '/'} className="w-full bg-brand-500 hover:bg-brand-600 text-white py-3 rounded-xl font-semibold transition">Go to Dashboard</button>button>
                        </div>div>
                </div>div>
              );
  }
    ...
    return (
          <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-brand-900 flex flex-col">
                <header className="px-6 py-5 flex items-center gap-3 border-b border-slate-700/50">
                        <div className="bg-brand-500 p-2 rounded-lg"><Truck className="w-5 h-5 text-white" /></div>div>
                        <div><span className="text-lg font-bold text-white">HaulFlow</span>span><span className="ml-2 text-sm text-slate-400">Self-Service Setup</span>span></div>div>
                </header>header>
                        ...
                        return (
                                        <div key={step.id} className="flex items-center flex-1 last:flex-none">
                                                        <div className="flex flex-col items-center">
                                                                          ...
                                                                          <span className={`mt-2 text-xs font-medium hidden sm:block ${isActive ? 'text-brand-300' : isDone ? 'text-emerald-400' : 'text-slate-500'}`}>{step.label}</span>span>
                                                        </div>div>
                ...
                <div className="flex-1 px-6 pb-8 max-w-3xl mx-auto w-full">
                        ...
                        <div className="px-6 sm:px-8 pb-2">
                                      <p className="text-red-400 text-sm">{saveError}</p>p>
                        </div>div>
                                  )}
