    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-brand-900 flex items-center justify-center p-4">
        <div className="bg-slate-800 border border-slate-700 rounded-2xl shadow-2xl p-10 w-full max-w-md text-center">
          ...
          <p className="text-slate-400 mb-6">Your HaulFlow account has been configured.</p>
      ...
      <div className="flex-1 px-6 pb-8 max-w-3xl mx-auto w-full">
        <div className="bg-slate-800 border border-slate-700 rounded-2xl shadow-2xl overflow-hidden">
          <div className="p-6 sm:p-8">
            {currentStep === 1 && <StepCompanyProfile data={companyProfile} onChange={setCompanyProfile} />}
            {currentStep === 2 && <StepDataImport files={uploadedFiles} onFilesChange={setUploadedFiles} />}
            {currentStep === 3 && <StepTeamInvites members={teamMembers} onChange={setTeamMembers} />}
            {currentStep === 4 && <StepInvoiceConfig data={invoiceConfig} onChange={setInvoiceConfig} />}
          </div>
          <div className="px-6 sm:px-8 py-5 bg-slate-800/50 border-t border-slate-700 flex items-center justify-between">
            <button onClick={handleBack} disabled={currentStep === 1} className="flex items-center gap-2 px-5 py-2.5 rounded-lg text-sm font-medium text-slate-300 hover:text-white hover:bg-slate-700 transition disabled:opacity-30 disabled:cursor-not-allowed"><ArrowLeft className="w-4 h-4" />Back</button>
            <div className="flex items-center gap-3">
              {currentStep < 4 && <button onClick={handleNext} className="text-sm text-slate-400 hover:text-slate-200 transition">Skip</button>}
              <button onClick={handleNext} disabled={!canProceed()} className="flex items-center gap-2 px-6 py-2.5 bg-brand-500 hover:bg-brand-600 text-white rounded-lg text-sm font-semibold transition disabled:opacity-50 disabled:cursor-not-allowed">{currentStep === 4 ? 'Finish Setup' : 'Continue'}<ArrowRight className="w-4 h-4" /></button>
            </div>
          </div>
        </div>
      </div>
