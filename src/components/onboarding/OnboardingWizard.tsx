...
// Steps after Stripe payment — company info already collected on /onboard
const STEPS = [
...
export default function OnboardingWizard() {
  ...
  // Pre-populate from data already collected during /onboard signup
  ...
  useEffect(() => {
    try {
      const stored = localStorage.getItem('hf_company');
      if (stored) {
        const co = JSON.parse(stored);
        setCompanyProfile({
          legalName:          co.name        || '',
          dotNumber:          co.dot_number  || '',
          mcNumber:           co.mc_number   || '',
          primaryHubAddress:  co.address     || '',
          taxId:              co.tax_id      || '',
        });
      }
    } catch (_) {}
  ...
  // Pre-populate invoice config billing info from stored user/company
  useEffect(() => {
    try {
      ...
      setInvoiceConfig(prev => ({
        ...prev,
        billingEmail: prev.billingEmail || company?.company_email || user?.email || '',
        billingPhone: prev.billingPhone || company?.company_phone || '',
      }));
    } catch (_) {}
  }, []);
  ...
  const canProceed = (): boolean => {
    switch (currentStep) {
      case 1: return true; // Data import is optional
      case 2: return true; // Team invites optional
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-brand-900 flex flex-col">
      <header className="px-6 py-5 flex items-center gap-3 border-b border-slate-700/50">
        ...
        <div>
          ...
          <span className="ml-2 text-sm text-slate-400">Finish Setup</span>
        </div>
      </header>
              ...
              <div key={step.id} className="flex items-center flex-1 last:flex-none">
                <div className="flex flex-col items-center">
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center transition-all duration-200 ${
                    ...
                    isActive ? 'bg-brand-500 text-white ring-4 ring-brand-500/30' :
                ...
                {idx < STEPS.length - 1 && (
                  <div className={`flex-1 h-0.5 mx-3 rounded transition-colors duration-200 ${
      ...
      <div className="flex-1 px-6 pb-8 max-w-3xl mx-auto w-full">
        <div className="bg-slate-800 border border-slate-700 rounded-2xl shadow-2xl overflow-hidden">
          ...
          <div className="px-6 sm:px-8 pb-6 sm:pb-8 flex justify-between items-center border-t border-slate-700 pt-6">
            ...
            >
              {saving ? 'Saving...' : currentStep === STEPS.length ? 'Finish Setup' : 'Next'}
